<template>
  <tbody>
    <tr v-for="product in filteredItems" :key="product.id" v-show="(showOnlyRunout && isRunout(product) || !showOnlyRunout)">
      ...
      <td v-show="columns.find(column => column.name == 'profit') && columns.find(column => column.name == 'profit').show" class="text-right">{{profit(product) | toCurrency}}</td>
...
export default {
  ...
  methods: {
    ...
    profit(product) {
      let productFromStore = this.$store.state.products.find(p => p.id == product.id)
