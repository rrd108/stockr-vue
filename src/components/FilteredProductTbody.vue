<template>
  <tbody>
    <tr v-for="product in filteredItems" :key="product.id" v-show="(showOnlyRunout && isRunout(product)) || !showOnlyRunout">
      ...
      <td
        v-show="
          columns.find(column => column.name == 'purchases') && columns.find(column => column.name == 'purchases').show
        "
        class="text-right"
      >
      ...
      <td
        v-show="columns.find(column => column.name == 'sells') && columns.find(column => column.name == 'sells').show"
        class="text-right"
      >
      ...
      <td
        v-show="columns.find(column => column.name == 'profit') && columns.find(column => column.name == 'profit').show"
        class="text-right"
      >
      ...
      <td
        v-show="columns.find(column => column.name == 'runout') && columns.find(column => column.name == 'runout').show"
        class="text-right"
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      profit(product) {
        let productFromStore = this.$store.products.find(p => p.id == product.id)
        return product.sellsIncome - productFromStore.lastPurchasePrice * -1 * product.sells
      },
      runout(product) {
        if (product.stock <= 0) {
          return new Date()
        ...
        if (product.sells < 0) {
          ...
          let runoutDays = parseInt(product.stock / (-product.sells / product.sellDays))
